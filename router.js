...
router.post('/', function (req, res, next) {
  ...
  //if user is registering, it will create a new user on DB and redirect to the profile
  if (req.body.email &&
    req.body.username &&
    req.body.password &&
    ...
    .exec(function (error, user) {
      if (error) {
        ...
        if (user === null) {
          ...
          return res.send('<h2>Name: </h2>' + user.username + '<h2>Mail: </h2>' + user.email + '<br><a type="button" href="/logout">Logout</a>')
        }
      }
    });
});
